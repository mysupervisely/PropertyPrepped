...
// Milestone 7 (renamed/repositioned in Investment Tools 2.0): Rental
// Property Analyzer — formerly "Property Evaluator." Same route content,
// same calculations, same saved-analysis data shape; only the name,
// header copy, and the address field (Part 1 bug fix) changed. The route
// moved from /investment-tools/property-evaluator to
// /investment-tools/rental-analyzer — the old path now redirects here
// (see app/investment-tools/property-evaluator/page.tsx) so existing
// bookmarks/links with ?analysisId=... keep working.
//
...
function PropertyEvaluator() {
  ...
  // Section 6/8/16: the Rental Property Analyzer itself is free on every plan —
  ...
  async function saveAnalysis() {
    ...
    if (savedId) {
      ...
      if (insertError) setError(insertError.message)
        ...
        router.replace(`/investment-tools/rental-analyzer?analysisId=${data.id}`)
      }
    }
    setSaving(false)
  }
