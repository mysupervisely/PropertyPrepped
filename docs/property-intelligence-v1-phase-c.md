...
### Follow-up refinement — mortgage note removal, visible Performance section, install-banner architecture fix (PR #64, third real-device pass)

The quiet-tile design above was reviewed on a real iPhone and **approved as final** — no further tile redesign. Real-device review found three remaining issues, all addressed here without touching the approved tile visuals, the engine, or any calculation.

**1. Mortgage explanation removed from the UI.** The engine's own explanatory note for a `financing_status_confirmed` `$0` (`{performance.mortgageBalance.source === 'financing_status_confirmed' && <p className="propertyPerformanceNote">{performance.mortgageBalance.notes?.[0]}</p>}`) is no longer rendered anywhere on the card — it's internal data-quality/engineering context a landlord doesn't need repeated every time; the "Paid Off"/"No Mortgage" primary tile already says what matters. The engine's `Metric.notes` field itself is untouched — only this one JSX line reading it was removed. The `potentiallyStale` note for an *actual* mortgage balance (a different, still-relevant caveat) is unaffected.

**2-3. Cap Rate and Net Cash Flow are now visible in the main snapshot.** A new "Performance" section sits between "YTD Performance" and the Purchase/Appreciation line — same `.propertySnapshotMetric`/`.propertySnapshotSecondaryGrid` tile pattern as YTD (two columns via a small `.propertySnapshotPerformanceGrid` override, slightly smaller type so it reads as a visible step quieter than both the primary and YTD tiles above it). Both figures are read verbatim from `priorYearPerformance` — the same `selectPriorYearPerformance()` result Phase C.2 already computed, never the current-year `performance`, never annualized, never a revived rent-minus-expenses formula. **No engine change was needed**: both metrics were already fully computed, just not yet rendered outside "View performance." (`resolveNetCashFlow()` was re-read to confirm `netCashFlowMonthly` is a genuinely monthly figure — `noiAnnual.value / 12 - monthlyDebtService.value` — so the existing `/mo` suffix is honest, not a silent unit conversion.) When `priorYearPerformance` exists, the heading reads "Performance · {year}"; when it doesn't, both tiles show a quiet "—" and a single shared note underneath ("Full-year data needed for annual performance.") — never a fabricated 0%/$0, never repeated per tile.

Because these two figures are now visible above the fold, "View performance" no longer repeats them — it was rewritten to hold only Full-Year Performance's Annual NOI (the one genuinely additional figure the visible section doesn't show) and Contract Annual Rent. The disclosure itself, and its accessibility (native `<details>/<summary>`, zero new JS), are unchanged.

**4. Install banner: architectural fix, not another clearance calculation.** Multiple real iPhone/Safari screenshots showed the Phase C.2 measured-clearance banner (`position: fixed`, a `ResizeObserver`-published `--install-hint-height` custom property, a `hasInstallHint` body class) **still** covering real product content — Dashboard maintenance cards, the Property Snapshot, Expenses & tax. A fixed overlay computing its own clearance is fundamentally fragile (Dynamic Type, viewport quirks, measurement timing), so the fix is structural rather than a better offset: `<InstallPrompt />` moved from being rendered as a sibling of `<body>` in `app/layout.tsx` (an unusual position that made `position: fixed` seem like the only way to place it usefully) to a plain child *inside* `<body>`, after `{children}`. It is now an ordinary, non-fixed flex block at the end of every page's normal document flow — it scrolls with the page and cannot cover anything, by construction, not by calculation.

Every mechanism that existed solely to support the old fixed-overlay clearance was removed as obsolete: `InstallPrompt.tsx`'s second `useEffect` (the `ResizeObserver`, the `hasInstallHint` toggle, the `--install-hint-height` custom property), the `hintRef`/`useRef` import, and `app/globals.css`'s `body.hasInstallHint .shell`/`body.hasBottomNav.hasInstallHint .shell` rules. `.installHint` itself dropped `position: fixed` for ordinary `margin` (a `max(16px, env(safe-area-inset-*))` safe-area-aware trailing margin, not a fixed-position offset); a plain, static (not measured) `margin-bottom` gives it breathing room above the fixed bottom nav on narrow viewports, deliberately not exact — ordinary in-flow whitespace at the very bottom of a scrollable page costs nothing.

**Kept, deliberately**: `body.hasBottomNav .shell`'s own bottom padding (still reads `var(--bottom-nav-height, 64px)`, `MobileBottomNav.tsx`'s `ResizeObserver` measurement) — independently required for the shell's clearance from the still-fixed bottom nav, and completely unrelated to the install-hint fix. The bottom nav itself (`.mobileBottomNav`, still `position: fixed`, still 4 destinations) was not touched. Dismiss/install functionality, copy ("Install PropRoster: tap Share → Add to Home Screen."), and classNames are unchanged — this is a placement fix, not a redesign.

**A narrow-breakpoint sizing detail caught during this pass**: the new Performance tiles share `.propertySnapshotSecondaryGrid` with the YTD tiles (to inherit their sizing "for free"), which meant the existing `≤360px` rule resetting YTD tiles to 14px would also reset Performance tiles to the same 14px — losing the intended "one step quieter than YTD" hierarchy at the narrowest breakpoint. Fixed with a scoped `.propertySnapshotPerformanceGrid .propertySnapshotMetric strong { font-size: 13px; }` override placed after the shared rule (equal specificity, later wins the cascade).

### Tests

`lib/dashboard/property-intelligence-v1-phase-c-wiring.test.ts`, `property-intelligence-v1-phase-c2-wiring.test.ts`, `property-snapshot-compact-v1-wiring.test.ts` — rescoped in place (repo convention: protect the same invariant with updated literals, never leave a test silently broken) for: the mortgage note's removal (flipped from "note renders" to "note is absent"), Cap Rate/Net Cash Flow's move from ternary-with-else to value-first `? … : '—'` JSX and the new tile count, "View performance"'s deduplication (`{priorYearPerformance && (...)}`, no more Cap Rate/Net Cash Flow inside it), and a full rewrite of the install-banner describe block for the new normal-flow architecture (no ResizeObserver/hasInstallHint/`--install-hint-height` anywhere, `<InstallPrompt />` confirmed positioned after `{children}` inside `<body>`, `.installHint` confirmed not `position: fixed`). New tests were added covering the visible Performance section directly: both tiles exist and are sourced from `priorYearPerformance.capRatePercent`/`netCashFlowMonthly` (never the current-year `performance`, never an annualizing formula, never the retired rent-minus-expenses calculation), the completed-year label, the "—"/shared-note behavior when no qualifying year exists, and the narrow-breakpoint font-size override.

Targeted run (`lib/dashboard/`): all passing (119 tests across the 5 directly-affected files). Full suite run once (required — shared install-layout behavior changed): all passing. `npx tsc --noEmit` and `npm run build` both clean.

### Visual QA

Scenario A (no qualifying completed year) and Scenario B (a qualifying prior completed year, 2025) were rendered in a static HTML mirror of the exact JSX/CSS classes, using the user's exact realistic data (Estimated Value $350,000, Estimated Equity $350,000, Monthly Rent $2,350, Mortgage: Paid Off, YTD Income $2,350/Expenses $500/NOI $1,850, Purchase Price $150,000), screenshotted at 320/390/430/1280px. No horizontal overflow at any width. Confirmed: the mortgage explanation sentence is gone; the Paid Off tile still displays correctly; Cap Rate/Net Cash Flow render as "—" with the shared note in Scenario A and real values with the "· 2025" label in Scenario B; primary/YTD tiles and the purchase/appreciation line are pixel-for-pixel the same approved design; "View performance" still opens and now shows only Annual NOI + Contract Annual Rent. A Dashboard-like page and a longer property-page mockup with the install banner rendered at the end confirmed it sits in normal document flow below all product content (including a "Needs Your Attention"-style card) and does not overlap anything, at every tested width.

### Remaining duplication / UX limitations

None identified. This was the second real-device-driven refinement pass on an otherwise-approved design; no new data/calculation limitation was introduced or discovered.
