...
## 12. Recorded for Phase E (found during Phase B/B.1 real-device review — not implemented yet)

**Recent Activity.** Should move to underneath My Properties (already adjacent — no structural move needed, just a re-read once Phase E touches this area), become collapsible, and default to **collapsed on mobile** (no existing signal argues for a different default — it's historical/reference information, lower priority than PropWatch or My Properties). Use the same compact Show/Hide toggle treatment `.snapshotToggle`/PropWatch's own collapse control already establishes — not a new interaction pattern.

**PropWatch hierarchy.** Real-device review found PropWatch currently stacks too many competing elements at once: the "PropWatch" eyebrow, a large "Stay ahead of what needs attention." heading, a separate Show/Hide control, an item count, and the Needs Your Attention card itself, all visible together. Phase E should simplify this specific hierarchy — fewer simultaneous visual elements, clearer single focal point. Not redesigned in Phase B/B.1; recorded here only so it isn't lost before Phase E.

## 13. Phase B.1 — authenticated mobile left-edge clipping (addendum)

A follow-up, narrower-scoped fix landed between Phase B and Phase C: real-device iPhone testing found the left-edge clipping bug (previously diagnosed and fixed once for `.topbar` specifically, see `docs/mobile-auth-layout-reliability-v1.md`) still recurring intermittently post-sign-in. Root cause and fix: `.shell` (the authenticated page's own root container) gained `overflow-x: hidden` — the same pattern already proven for `.tenantPortalShell` — so the document itself can no longer become horizontally scrollable while authenticated, regardless of which specific child causes a transient overflow; plus an explicit scroll-position reset (both axes) on the sign-in transition and on the two pre-existing "scroll to top" call sites, since sign-in swaps JSX within the same mounted document rather than navigating, so a stale horizontal scroll position could otherwise carry straight through. See the commit for full detail — recorded here only so Phase C/D/E don't rediscover or accidentally revert it.
